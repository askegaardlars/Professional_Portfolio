...
dat:				@ global variable dat of type struct try
	...
	.word 0 		@ member subct of type int
	.word 0			@ member linect of type int
	.word 0			@ member inchar2 of type int
	.word 0			@ member outchar2 of type int
	...
	_inchar2 = 24	@ _inchar2 is offset for member inchar2
	_outchar2 = 28	@_outchar2 is offset for member outchar2
...
post_linect:
	...
	cmp	r0, r1			
	bne	check_2nd_char		@ then jump to check for the second char
...
check_2nd_char:																						/////////////
	ldr	r1, [r4, #_inchar2]	@ if buff[index] != inchar2	

	cmp	r0, r1			
	bne	loopend				@ then jump to loopend
...
gettrans:
	...
	cmp	r0, #48			@ if second byte is 0
	beq	return0			@ return 0

	cmp	r0, #32			@ if second byte is space
	beq	return0			@ then begin exit from gettrans with branch to return0
	ldr	r0, [fp, #-12]	@ begin call to get_byte
	...
	cmp	r0, #32				@ if third character is not a space
	bne	return0				@ return null
	...
	cmp	r0, #10				@ if fourth character is \n
	beq	return0				@ then begin exit from gettrans with branch to return0
	
	cmp	r0, #0				@ if fourth character is null
	beq	return0				@ return 0

	cmp r0, #32				@ if fourth character is space
	beq	return0				@ return 0

	cmp	r0, #48				@ if fourth character is 0
	beq	return0				@ return 0
	...
	cmp	r0, #10				@ if fifth character is \n
	beq	return0				@ then begin exit from gettrans with branch to return0
	
	cmp	r0, #0				@ if fifth character is null
	beq	return0				@ return 0

	cmp r0, #32				@ if fifth character is space
	beq	return0				@ return 0

	cmp	r0, #48				@ if fifth character is 0
	beq	return0				@ return 0
	...
	mov	r1, #5
	bl	get_byte
	@r0 holds  byte stored at buff[5]
 
	cmp	r0, #10				@ if 6th character is not null
	...
	ldr	r0, [fp, #-12]
	mov	r1, #1				@ copy 2nd character into inchar2
	bl	get_byte
	@ r0 holds the byte at buff[1]

	str	r0, [r4, #_inchar2]		@inchar2 holds 2nd char of buff
	...
	mov 	r1, #3			
	bl 	get_byte
	@ r0 holds the byte at buff[3]

	str r0, [r4, #_outchar]	@ outchar holds the byte at buff[4]


	ldr	r0, [fp, #-12]
	mov	r1, #4				@ copy 5th character into inchar2
	bl	get_byte
	@ r0 holds the byte at buff[4]

	str	r0, [r4, #_outchar2]	@outchar2 holds 5th char of buff
...
printgettrans:				@ format string for gettrans printf
	...
	.align	2
printgettrans_second:		@ format string for gettrans printf
	.asciz 	"%c%c\n"
...
main:	
	...
	ldr	r1, [r4, #_inchar2]	@ begin call to print buff after gettrans
	ldr	r2, [r4, #_outchar2]
	ldr	r0, printgettrans_secondP
	bl	printf
	@ return r0 is number of printed characters from printf
...
printgettransP:
	...
	.align	2
printgettrans_secondP:
	.word	printgettrans_second
